...
var loaderUtils__namespace = /*#__PURE__*/_interopNamespace(loaderUtils);
var schemaUtils__namespace = /*#__PURE__*/_interopNamespace(schemaUtils);
...
function getStimulusCommentOptions (source) {
    let comments;
    try {
    ...
    for (const comment of comments) {
        ...
        if (value && stimulusCommentRegExp.test(value)) {
            try {
                const val = vm__default["default"].runInNewContext(`(function(){return {${value}};})()`);
...
function lazyControllerLoader (source, sourceMap) {
    ...
    if (!['eager', 'lazy'].includes(stimulusFetch)) {
        this.emitError(new Error(`Invalid value "${stimulusFetch}" found for "stimulusFetch". Allowed values are "lazy" or "eager"`));
    }
    const loaderOptions = loaderUtils__namespace.getOptions(this);
    schemaUtils__namespace.validate(schema, loaderOptions, {
...
module.exports = lazyControllerLoader;
