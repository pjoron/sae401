  ...
  return {
    ...
    warnings: result.warnings.length > 0 ? result.warnings.map(item => {
      return {
        ...
        line: item.location && item.location.line ? item.location.line : // eslint-disable-next-line no-undefined
        undefined,
        column: item.location && item.location.column ? item.location.column : // eslint-disable-next-line no-undefined
        undefined,
...
async function parcelCssMinify(input, sourceMap, minimizerOptions) {
  ...
  const parcelCss = require("@parcel/css"); // Copy `parcel-css` options
...
async function lightningCssMinify(input, sourceMap, minimizerOptions) {
  ...
  const lightningCss = require("lightningcss"); // Copy `lightningCss` options


  const lightningCssOptions = buildLightningCssOptions(minimizerOptions); // Let `esbuild` generate a SourceMap
...
async function swcMinify(input, sourceMap, minimizerOptions) {
  ...
  const swc = require("@swc/css"); // Copy `swc` options
